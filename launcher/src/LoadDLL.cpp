...
extern "C" void kfree(void* address);
...
typedef struct LOAD_DLL_CONTEXT
{
	...
	IMPORT_MODULE* loaded_import_modules_array;
...
static ELoadDLLResult LoadDLL_LoadHeaders(LOAD_DLL_CONTEXT* ctx, LOAD_DLL_READPROC read_proc, void* read_proc_param)
{
	...
	ctx->sect = (IMAGE_SECTION_HEADER*)kmalloc(ctx->size_section_headers);
...
static ELoadDLLResult LoadDLL_AllocateMemory(LOAD_DLL_CONTEXT* ctx, int flags)
{
	...
	*/
	/*LOG(LOG_INFO, "0x%x, 0x%x\n", ctx->hdr.OptionalHeader.ImageBase, rva_high - rva_low);
	...
	*/
	ctx->image = (void*)ctx->hdr.OptionalHeader.ImageBase;
	if (!ctx->image)
	{
		/*
...
/*static ELoadDLLResult LoadDLL_ResolveImports(LOAD_DLL_CONTEXT* ctx)
{
	...
	for (; import_desc->Name; ++import_desc)
	{
		...
		if (ctx->num_import_modules >= ctx->import_modules_array_capacity)
		{
			...
			new_module_array = (HMODULE*)kmalloc(sizeof(HMODULE)*ctx->import_modules_array_capacity);
			if (!new_module_array)
				return ELoadDLLResult_MemoryAllocationError;
...
static ELoadDLLResult LoadDLL_ResolveImports(LOAD_DLL_CONTEXT* ctx)
{
	...
	DWORD_PTR* src_iat;
	DWORD_PTR* dest_iat;
	IMPORT_MODULE* new_module_array;
	...
	for (; import_desc->Name; ++import_desc)
	{
		char* moduleName = (char*)(ctx->image_base + import_desc->Name);
		hDLL = ModuleManager::GetInstance()->GetSystemPE(moduleName);

		if(!hDLL)
			hDLL = ModuleManager::GetInstance()->LoadPE(moduleName);
		...
		{
			ctx->import_modules_array_capacity = ctx->import_modules_array_capacity ? ctx->import_modules_array_capacity * 2 : 16;
			new_module_array = (IMPORT_MODULE*)kmalloc(sizeof(IMPORT_MODULE)*ctx->import_modules_array_capacity);
			if (!new_module_array)
				return ELoadDLLResult_MemoryAllocationError;
			if (ctx->num_import_modules)
				memcpy(new_module_array, ctx->loaded_import_modules_array, sizeof(IMPORT_MODULE)*ctx->num_import_modules);
			kfree(ctx->loaded_import_modules_array);
			ctx->loaded_import_modules_array = new_module_array;
		}
		new_module_array[ctx->num_import_modules].image_base = hDLL;
		strcpy(new_module_array[ctx->num_import_modules].moduleName, (char*)(ctx->image_base + import_desc->Name));
		...
		for (; *src_iat; ++src_iat, ++dest_iat)
		{
			...
			void* p = ModuleManager::GetInstance()->GetModuleFunction(hDLL, (char*)pname->Name);
...
static ELoadDLLResult LoadDLL_SetSectionMemoryProtection(LOAD_DLL_CONTEXT* ctx)
{
	...
	*/
	...
	for (i=0,s=ctx->sect; i<ctx->hdr.FileHeader.NumberOfSections; ++i,++s)
	{
		...
		if (!VirtualProtect((LPVOID)(ctx->image_base + s->VirtualAddress), s->Misc.VirtualSize, protection, &oldProtection))
		...
		//	try
			...
			//{
				...
				if (!finished_successfully || !info)
				{
					...
					{						
						for (int i = 0; i < (int)ctx.num_import_modules; ++i)
							UnloadDLL((LOAD_DLL_INFO*)(ctx.loaded_import_modules_array[i].image_base));
...
bool UnloadDLL(LOAD_DLL_INFO* info)
{
	...
	{
		for (int i = 0; i < (int)info->num_import_modules; ++i)
		{
			UnloadDLL((LOAD_DLL_INFO*)(info->loaded_import_modules_array[i].image_base));
		}
