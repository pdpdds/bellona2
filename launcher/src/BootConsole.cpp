...
extern "C" void OutPortByte(uint16_t port, uint8_t value);
...
namespace BootConsole
{
	...
	static uint32_t m_xPos;
	static uint32_t m_yPos;

	static uint16_t* m_pVideoMemory; //Just a pointer to video memory
	...
	void Clear()
	{
		...
		for (uint32_t i = 0; i < m_ScreenWidth * m_ScreenHeight; i++)				//Remember, 25 rows and 80 columns
		{
			m_pVideoMemory[i] = (uint16_t)(0x20 | (m_Color << 8));
		}
	...
	void WriteChar(char c, ConsoleColor textColour, ConsoleColor backColour)
	{
		...
		default:						// -> all other characters
			...
			uint16_t* VideoMemory = m_pVideoMemory + m_ScreenWidth * m_yPos + m_xPos;
			uint8_t attribute = (uint8_t)((backColour << 4) | (textColour & 0xF));

			*VideoMemory = (c | (uint16_t)(attribute << 8));
			m_xPos++;
			break;
		}
	...
	void WriteString(const char* szString, ConsoleColor textColour, ConsoleColor backColour)
	{
		...
		for (int i = 0; szString[i] != 0; i++)
		{
			...
			*VideoMemory = (szString[i] | (uint16_t)(attribute << 8));
		}
	...
	void GetCursorPos(uint32_t& x, uint32_t& y) { x = m_xPos; y = m_yPos; }
