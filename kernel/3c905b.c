...
static int select_register_window_3c905b( nWindow )
{
	DWORD dwPort, dwX;
...
static void write_3c905b_register( int nRegName, DWORD dwValue )
{
	...
	// write port
	if( pR->nSize == 1 )
		vWritePort( dwPort, dwValue );
	else if( pR->nSize == 2 )
		vWritePortWord( dwPort, dwValue );
	else
		vWritePortDword( dwPort, dwValue );
}
...
static DWORD read_3c905b_register( int nRegName )
{
	...
	// read port
	if( pR->nSize == 1 )
		vReadPort( dwPort, (UCHAR*)&dwX );
	else if( pR->nSize == 2 )
		vReadPortWord( dwPort, (UINT16*)&dwX );
	else
		vReadPortDword( dwPort, &dwX );
...
static int issue_3c905b_command( UINT16 wCommandValue )
{
	...
	dwPort = ( pci_3c905b.cfg.base_addr[0] & (DWORD)0xFFFFFF80 ) + (DWORD)0x0E;
	vWritePortWord( dwPort, wCommandValue );
